...
/**
 * Mean-pool a document's chunk embeddings into one query vector — reuses the
 * embeddings already paid for at ingestion (used for both mapping's kNN
 * candidate generation and cross-source duplicate detection). Returns null if
 * the doc has no embedded chunks yet (nothing to compare against).
 */
